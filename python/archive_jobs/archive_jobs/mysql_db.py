...
'''
skrypt uzywany do łączenia się z bazą danych i wykonywania operacji na tabelach
tworenie tabel, bazy danych
wyciągania i updatowanie danych
...
class table_management: #do zarządzania istniejącymi tabelami
    ...
    def add_data_row_or_update(self, table_name, dictionary):
        """ funkcja zapisuje nowy wiersz w DB albo jeśli istnieje to aktualizuje
         Warunek: > musi być stworzony unique key i primary key
         %s pomaga przy znakach typu ' i " bo wtedy nie wyskakuje błąd
         """
        col_names = "(" + ",".join([str(i) for i in dictionary.keys()]) + ")"
        double_values = list(dictionary.values()) * 2  # podwójna lista bo dwa razy %s dajemy
        values_string = "(" + ", ".join(["%s"] * len(dictionary)) + ")"

        string_lst = []
        for column, value in dictionary.items():
            string_lst.append(f"{column}=%s")
        update_statement = ','.join(string_lst)

        sql_st = f"INSERT INTO {table_name} {col_names} VALUES {values_string} ON DUPLICATE KEY UPDATE {update_statement}"  #podajemy (%s, %s ..) dlatego, że w ten sposób mozemy przekazać null values
        self.mycursor.execute(sql_st, double_values)
        self.db.commit()
    ...
    def fetch_result_custom(self, table_name, columns, filt_col_statement): #pobieranie wszystkich wyników tabeli w formie listy
        self.mycursor.execute(f'select {columns} from {table_name} {filt_col_statement}')
        result = self.mycursor.fetchall()
        return result

    def fetch_result_fully_customized(self, statement):
        self.mycursor.execute(statement)
        result = self.mycursor.fetchall()
        return result
